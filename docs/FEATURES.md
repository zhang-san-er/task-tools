...
### 移动端优先设计
- **项目定位**：本项目主要面向移动端用户，特别是苹果手机（iPhone）用户
- **触摸优化**：所有交互均针对触摸操作优化
- **无悬停交互**：不考虑PC端的鼠标悬停（hover）交互，所有功能通过点击/触摸触发
- **点击反馈**：使用 `active:` 伪类提供触摸反馈，而非 `hover:` 伪类
...
- 次要操作：灰色背景
- 危险操作：红色背景
- 点击反馈：`active:scale-95`（移动端触摸反馈）
- 注意：不使用 `hover:` 伪类，因为移动端不支持悬停
