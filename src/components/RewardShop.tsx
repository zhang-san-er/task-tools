...
export const RewardShop: React.FC = () => {
  const { rewards, redeemReward, addReward, updateReward, deleteReward, toggleRewardStatus } = useRewardStore();
  ...
  const handleToggleStatus = (rewardId: string, currentStatus: boolean | undefined) => {
        ...
        <div className="glass-effect rounded-2xl card-shadow p-5 mb-4 border border-white/50">
          {showAddForm ? (
            <form onSubmit={handleAdd} className="space-y-3">
              <h4 className="font-bold text-gray-800 mb-3">添加新奖励</h4>
              <input
                type="text"
                name="name"
                placeholder="奖励名称"
      ...
      {editingReward && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <form
            ...
            <h3 className="text-lg font-black text-gray-800 mb-4">编辑奖励</h3>
        ...
        {displayRewards.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-sm">
              {isEditing ? '还没有商品，点击"添加新奖励"创建第一个商品' : '暂无上架商品'}
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <h4 className={`font-black ${isActive ? 'text-gray-800' : 'text-gray-400'}`}>{reward.name}</h4>
                    ...
                    <p className={`text-sm mb-3 ${isActive ? 'text-gray-600' : 'text-gray-400'}`}>{reward.description}</p>
                    <div className="flex items-center justify-between">
                      <div className={`text-lg font-black ${isActive ? 'text-orange-600' : 'text-gray-400'}`}>
                        {reward.cost} 积分
                      </div>
                      <div className="flex gap-2">
                        {isEditing && (
                          <>
                            <button
                              ...
                              className={`px-4 py-2 rounded-xl font-semibold text-sm transition-all shadow-sm active:scale-95 ${
                                isActive
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-blue-500 text-white rounded-xl font-semibold text-sm transition-all shadow-sm active:scale-95">
                              编辑
                            </button>
                            <button
                              onClick={() => handleDelete(reward.id, reward.name)}
                              className="px-4 py-2 bg-red-500 text-white rounded-xl font-semibold text-sm transition-all shadow-sm active:scale-95">
                              删除
                            </button>
                          </>
                        )}
